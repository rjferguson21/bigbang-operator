...
.PHONY: schema-to-go
schema-to-go: go-jsonschema ## Regenerate api/v1alpha1/zz_generated.types.go from bb-common's values.schema.json.
	./hack/schema-to-go.sh

.PHONY: chart-sync
chart-sync: manifests ## Mirror config/crd/bases into chart/templates/crds.
	./hack/chart-sync.sh
...
KIND_CLUSTER ?= bigbang-operator-test-e2e
...
docker-buildx: ## Build and push docker image for the manager for cross-platform support
	...
	- $(CONTAINER_TOOL) buildx create --name bigbang-operator-builder
	$(CONTAINER_TOOL) buildx use bigbang-operator-builder
	- $(CONTAINER_TOOL) buildx build --push --platform=$(PLATFORMS) --tag ${IMG} -f Dockerfile.cross .
	- $(CONTAINER_TOOL) buildx rm bigbang-operator-builder
	rm Dockerfile.cross
...
.PHONY: go-jsonschema
go-jsonschema: $(GO_JSONSCHEMA) ## Download go-jsonschema locally if necessary.
$(GO_JSONSCHEMA): $(LOCALBIN)
	$(call go-install-tool,$(GO_JSONSCHEMA),github.com/atombender/go-jsonschema,$(GO_JSONSCHEMA_VERSION))
